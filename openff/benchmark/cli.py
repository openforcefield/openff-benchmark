...
@click.option(
    ...
    help="Address and port of the QCFractal Server",
)
@click.option(
    "-d", "--dataset-name", required=True, help="Dataset name to submit molecules under"
)
@click.option(
    "--recursive",
    is_flag=True,
    help="Recursively traverse directories for SDF files to submit",
)
@click.option(
    "-s",
    "--season",
...
@click.option(
    ...
    "--dataset-name",
    required=True,
    help="Dataset name to export molecule optimization data from",
)
@click.option(
    "--delete-existing",
    is_flag=True,
...
@click.option(
    ...
    "--output-directory",
    required=True,
    help="Output directory to use for results",
)
...
@click.option(
    ...
    "--dataset-name",
    required=True,
    help="Dataset name to limit scope of status display to",
)
@click.option(
    "-c",
    "--compute-specs",
    default=None,
    help="Comma-separated compute spec names to limit status display to",
)
...
def status(molids, fractal_uri, dataset_name, compute_specs):
    ...
    optdf = optexec.get_optimization_status(
        fractal_uri, dataset_name, compute_specs=compute_specs, molids=molids
    )
...
@click.option(
    ...
    "--dataset-name",
    default=None,
    help="Dataset name to limit scope of progress display to",
)
@click.option(
    "-c",
    "--compute-specs",
    default=None,
    help="Comma-separated compute spec names to limit progress display to",
)
def progress(fractal_uri, frequency, dataset_name, compute_specs):
    ...
    for dataset in datasets:
        dfs.append(
            optexec.get_optimization_status(
                fractal_uri=fractal_uri,
    ...
    complete = sum(
        [
            len([opt for opt in df.values.flatten() if opt.status == "COMPLETE"])
            for df in dfs
        ]
    )
...
@click.option(
    "-d", "--dataset-name", required=True, help="Name of dataset to change priority of"
)
...
@click.option(
    "-d", "--dataset-name", required=True, help="Name of dataset to error cycle"
)
@click.option(
    "-c",
    "--compute-specs",
    default=None,
    help="Comma-separated compute spec names to limit errorcycling to",
)
@click.option(
    "-w",
    "--watch",
    default=None,
    help="Run in 'watch' mode with interval in seconds between cycles",
)
@click.argument("molids", nargs=-1)
def errorcycle(molids, fractal_uri, dataset_name, watch, compute_specs):
    ...
    if watch is not None:
        while True:
            optexec.errorcycle_optimizations(
                fractal_uri, dataset_name, compute_specs=compute_specs, molids=molids
            )
            sleep(watch)
    else:
...
@click.option(
    "-d", "--dataset-name", required=True, help="Name of dataset to pull tracebacks for"
)
@click.option(
    "-c",
    "--compute-specs",
    default=None,
    help="Comma-separated compute spec names to limit traceback reporting from",
)
@click.argument("molids", nargs=-1)
def traceback(molids, fractal_uri, dataset_name, compute_specs):
    ...
    tracebacks = optexec.get_optimization_tracebacks(
        fractal_uri, dataset_name, compute_specs=compute_specs, molids=molids
    )
...
@click.option(
    ...
    "--dataset-name",
    required=True,
    help="Name of dataset to extract raw molecule and optimization results from",
)
@click.option(
    "-c",
    "--compute-specs",
...
def extract(molids, fractal_uri, dataset_name, compute_specs):
    ...
    results = optexec.get_optimization_from_server(
        fractal_uri, dataset_name, compute_specs=compute_specs, molids=molids
    )
...
@click.option(
    ...
    type=int,
    default=200,
    help="Maximum iterations to allow for SCF convergence",
)
@click.option(
    "--geometric-maxiter",
    type=int,
    default=300,
    help="Maximum iterations to allow for geometry optimization convergence",
)
@click.option(
    "--geometric-coordsys",
    type=click.Choice(["dlc", "tric"]),
    default="dlc",
    help="Internal coordinate scheme to use for geometry optimization",
)
@click.option(
    "--geometric-qccnv",
    is_flag=True,
    help="If set, use QChem-style convergence criteria",
)
@click.argument("molids", nargs=-1)
def execute_from_server(
    ...
    results = optexec.execute_optimization_from_server(
        fractal_uri,
        dataset_name,
...
@click.option(
    ...
    "--nthreads",
    default=2,
    help="Number of threads to utilize for each gradient calculation step",
)
@click.option(
    "-m",
    "--memory",
    default=2,
    help="Maximum memory in GiB to allocate for each gradient calculation; set at least 5% *below* desired maximum",
)
@click.option(
    "--delete-existing",
    is_flag=True,
...
@click.option(
    "--recursive",
    is_flag=True,
    help="Recursively traverse directories for SDF files to include",
)
@click.option(
    "-o",
    "--output-directory",
...
def lucas(input_path, ref_method, output_directory):
     ...
     new_analysis.lucas(input_path, ref_method, output_directory)
...
def swope(input_path, ref_method, output_directory):
    from .analysis import new_analysis
    new_analysis.swope(input_path, ref_method, output_directory)
...
def plots_lucas(input_path, output_directory):
    ...
    new_draw.plot_lucas(input_path, output_directory)
...
def plots_swope(input_path, de_cutoff, rmsd_cutoff, output_directory):
    from .analysis import new_draw
    new_draw.plot_swope(input_path, de_cutoff, rmsd_cutoff, output_directory)
...
@click.option(
    "--schrodinger-path",
    default=None,
    help="Path to schrodinger binaries. Defaults to $SCHRODINGER.",
)
@click.option(
    "--host", default="localhost", help="Host name as specified in SCHRODINGER_HOSTS."
)
@click.option(
    "--max-jobs",
    default=None,
    help="Maximal number of subjobs. Defaults to no limit on subjobs.",
)
@click.option(
    "--opls-dir",
    default=None,
    help="Path to the custom OPLSDIR. If it is not given, ALL custom parameters will be calculated from scratch.",
)
@click.option(
    "--recursive",
    is_flag=True,
...
@click.option(
    "-o",
    "--output-path",
    default="7-schrodinger_ffb",
    help="Path where output files are written to.",
)
@click.argument("input-path", nargs=-1)
def ffbuilder(
    input_path, schrodinger_path, host, max_jobs, opls_dir, recursive, output_path
):
    ...
    if opls_dir is None:
        opls_dir = None
        warnings.warn(
            "The OPLS custom parameter path is not given. All parameters are recalculated. "
...
@click.option(
    "--opls-dir",
    default=None,
    help="Path to the custom OPLSDIR. "
...
def ffmerge(schrodinger_path, opls_dir, input_path):
    ...
    if not os.path.isdir(input_path):
        raise ValueError(
            "No ffbuilder job output files in the specified input directory. "
            "Maybe the ffbuilder job is not yet finished."
        )
    if opls_dir is None:
        warnings.warn(
            "The specified OPLS custom parameter path is not given. A default path is used."
        )
        opls_dir = os.path.join(os.getenv("HOME"), ".schrodinger", "opls_dir")
        if not os.path.isdir(opls_dir):
            warnings.warn(
                "The default OPLS custom parameter path does not exist."
                "A new OPLS directory is initialized in $HOME/.schrodinger/opls_dir/."
            )
    else:
        if not os.path.isdir(opls_dir):
            warnings.warn(
                "The specified OPLS custom parameter path does not exist."
                "A new OPLS directory is initialized at the specified path."
            )

    ffbuilder.ffb_merge(
        schrodinger_path=schrodinger_path, opls_dir=opls_dir, input_path=input_path
    )
...
@click.option(
    "--opls-dir",
    default=None,
    help="Path to the custom OPLSDIR. If not specified, NO custom parameters will be used.",
)
@click.option(
    "--recursive",
    is_flag=True,
...
def optimize(
    ...
    if opls_dir is not None and not os.path.isdir(opls_dir):
        warnings.warn(
            "The specified OPLS custom parameter path is not found. Default parameters and no custom parameters will be used."
        )
        opls_dir = None
    ...
    if (not delete_existing) and os.path.exists(output_path):
        raise Exception(
            f"Output directory {output_path} exists. If you want to replace it, run this command with --delete-existing."
        )
...
@click.argument("input-3d-molecules", nargs=-1)
def validate(input_3d_molecules, output_directory, group_name, delete_existing, add):
    ...
    error_dir = prepare_folders(
        output_directory=output_directory, delete_existing=delete_existing, add=add
    )
    ...
    for molecule_3d_file in tqdm(input_3d_molecules):
        assert os.path.exists(
            molecule_3d_file
        ), f"File {molecule_3d_file} does not exist"
        ...
        try:
            ...
            loaded_mols = Molecule.from_file(
                molecule_3d_file, file_format="sdf", allow_undefined_stereo=True
            )
    ...
    output = validate_and_assign(
        input_mols, group_name, add, existing_output_mols, existing_name_assignments
    )
    ...
    for success_mol in tqdm(success_mols):
        success_mol.to_file(
            os.path.join(output_directory, success_mol.name + ".sdf"), "sdf"
        )
    ...
    if len(existing_error_mols) == 0:
        ...
        existing_error_mol_indices = [
            int(i.replace("error_mol_", "").replace(".sdf", ""))
            for i in existing_error_mols
        ]
    ...
    with open(
        os.path.join(output_directory, "name_assignments.csv"), "w", newline=""
    ) as of:
...
def generate_conformers(input_directory, output_directory, add, delete_existing):
    ...
    logging.basicConfig(filename="generate-conformers.log", level=logging.DEBUG)
    ...
    if add:
        ...
        output_files = set(
            [
                ...
                for path in glob.glob(os.path.join(output_directory, "*00.sdf"))
            ]
        )
        ...
        if not new_file_basenames:
            print(
                f"No new files found in {input_directory}, the coverage report was not changed."
            )
            return
        # now remake the file paths
        molecule_files = [
            os.path.join(input_directory, filename) for filename in new_file_basenames
        ]
    ...
    for input_file in molecule_files:
        ...
        match = re.findall("([a-zA-Z0-9]{3})-([0-9]{5})-([0-9]{2}).sdf", input_file)
        assert len(match) == 1
        group_id = match[0][0]
        mol_idx = match[0][1]
        conf_id = match[0][2]
...
@click.option(
    ...
    default=None,
    type=click.INT,
    help="Number of parellel processes to use to generate coverage report",
)
@click.option("--delete-existing", is_flag=True)
def coverage_report(
    input_directory, forcefield_name, output_directory, processors, delete_existing, add
):
    """
    ...
    logging.basicConfig(filename="coverage-report.log", level=logging.DEBUG)
    ...
    if add:
        ...
        # now remake the file paths
        molecule_files = [
            os.path.join(input_directory, filename) for filename in new_files
        ]
    ...
    molecules = [
        Molecule.from_file(mol_file, file_format="sdf", allow_undefined_stereo=True)
        for mol_file in molecule_files
    ]

    report, success_mols, error_mols = generate_coverage_report(
    ...
    if add:
        ...
        total_molecules = old_report.pop("total_unique_molecules") + report.pop(
            "total_unique_molecules"
        )
        passed_molecules = old_report.pop("passed_unique_molecules") + report.pop(
            "passed_unique_molecules"
        )
